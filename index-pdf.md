---
title: Workshop OpenGeo Suite
author: Anton Bakker and Bart van den Eijnden
abstract: A small adventure in map building with the OpenGeo Suite featuring GeoServer, PostGIS, OpenLayers 3 and het Nationaal Georegister
---
...
>
>
> Check out the [full demonstration application](http://arbakker.github.io/workshop-boundless-geocat/code/index.html) and play!
    ...
    BEGIN
       ...
       FOR col_name_row IN col_names LOOP
          IF  col_name_row.cn not in ('wk_code','wk_naam','gm_code','gm_naam','water', 'geom' ) THEN
             ...
             (-99999997,-99999998,-99999999)', col_name_row.cn, col_name_row.cn);
          END IF;
       END LOOP;
    END $$;
...
- Paste in the style definition (below) for [stddev.xml](http://arbakker.github.io/workshop-boundless-geocat/data/stddev.xml) and hit the Save button at the bottom
...
The first thing we need for our app is a data file that maps the short, unpractical column names in our census table to human readable information. Fortunately, the [dictionary.txt](http://arbakker.github.io/workshop-boundless-geocat/data/stddev.xml) file has all the information we need. The dictionary.txt file was created by copy pasting the text of the [Buurten en Wijken documentation pdf](http://download.cbs.nl/regionale-kaarten/toelichting-buurtkaart-2013-v1.pdf) in a [text file](http://arbakker.github.io/workshop-boundless-geocat/data/census_nl-fields.txt) and combining this with a [list](datahttp://arbakker.github.io/workshop-boundless-geocat/data/column_names.txt) of all the columns of the Buurten en Wijken dataset with a [python script](data/http://arbakker.github.io/workshop-boundless-geocat/data/extract-dictionary.py.txt).  
...
Here’s a couple example lines from the [dictionary.txt](http://arbakker.github.io/workshop-boundless-geocat/data/dictionary.txt) file :
      ...
      <body>
        ...
        <div id="map">
          ...
          <img class="legend img-rounded" src=
           "https://workshop-boundless-geocat.geocat.net/geoserver/opengeo/wms?REQUEST=
           GetLegendGraphic&VERSION=1.3.0&FORMAT=image/png&WIDTH=26&HEIGHT=18&
           STRICT=false&LAYER=normalized&LEGEND_OPTIONS=fontName:sans-serif;
           fontSize:11;fontAntiAliasing:true;fontStyle:normal;fontColor:0xFFFFFF;
           bgColor:0x000000;">
        </div>
...
The real code is in the [wijkenkaart.js](http://arbakker.github.io/workshop-boundless-geocat/code/wijkenkaart.js) file. We start by creating an [Openbasiskaart](http://openbasiskaart.nl) base layer, and adding our parameterized census layer on top as an image layer with a [WMS Layer source](http://openlayers.org/en/master/apidoc/ol.source.ImageWMS.html).
    ...
    13.44,6.72,3.36,1.68,0.84,0.42,0.21];
...
The select element with the id topics will be our drop-down list of available columns. We load the [dictionary.txt](http://arbakker.github.io/workshop-boundless-geocat/data/dictionary.txt) file, and fill the select element with its contents. This is done by adding an option child for each line.
...
Look at the the [wijkenkaart.js](http://arbakker.github.io/workshop-boundless-geocat/code/wijkenkaart.js) file to see the whole application in one page.

When we open the [index.html](http://arbakker.github.io/workshop-boundless-geocat/code/index.html) file, we see the application in action.
...
Workshop created for [GeoBuzz 2014](http://geobuzz.nl/) by [GeoCat](http://www.geocat.net) and [Boundless](http://www.boundlessgeo.com). Workshop heavily inspired/copied from [Building a Census Map](http://workshops.boundlessgeo.com/tutorial-censusmap/).

Workshop licensed under Creative Commons [by-nc-sa](http://creativecommons.org/licenses/by-nc-sa/3.0/).

Contact: [Anton Bakker](mailto:anton.bakker@geocat.net) and [Bart van den Eijnden](mailto:bartvde@boundless.com).

![geocat](https://www.geocat.net/wp-content/uploads/2014/11/GeoCat_500x1061.png)
![boundless](http://boundlessgeo.com/wp-content/uploads/2013/09/Boundless_color.png)
